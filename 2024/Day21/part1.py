...
def preferenced_order(
    ...
    if len(memo) <= chain_length:
        for chain_len in range(len(memo) - 1, chain_length):
            ...
            for s in ["^>", "^<", "v>", "v<"]:
                if len(
                    operate_keypad(KeypadType.DIRECTIONAL, s + "A", chain_len)
                ) <= len(
                    operate_keypad(KeypadType.DIRECTIONAL, s[::-1] + "A", chain_len)
                ):
                    memo[-1].append(s)
...
def operate_keypad(keypad_type: KeypadType, code: str, chain_length: int) -> str:
    previous: str = "A"
    answer: list[str] = []
    ...
    for button in code:
        ...
        if distance[1] == 0:
            if distance[0] > 0:
                answer.append("v" * distance[0])
            else:
                answer.append("^" * (-distance[0]))
        elif distance[0] == 0:
            if distance[1] > 0:
    ...
    result: str = "".join(answer)
    if chain_length == 0:
        return result
    return operate_keypad(KeypadType.DIRECTIONAL, result, chain_length - 1)
...
def main() -> None:
    ...
    for code in codes:
        ...
        code = operate_keypad(KeypadType.NUMERIC, code, 2)
        answer += len(code) * numeric_part
    print(answer)

...
def hacked_neighbors(
    vertex: Vertex, board: list[str]
) -> Generator[Tuple[Vertex, int], None, None]:
    MAX_DISTANCE: Final = 20
    neighbor: Vertex
    for r in range(-MAX_DISTANCE, MAX_DISTANCE + 1):
        for c in range(-MAX_DISTANCE + abs(r), MAX_DISTANCE - abs(r) + 1):
            neighbor = move(vertex, (r, c))
            if not 0 <= neighbor[0] < len(board):
                continue
            if not 0 <= neighbor[1] < len(board[neighbor[0]]):
                continue
            if board[neighbor[0]][neighbor[1]] != "#":
                yield neighbor, abs(r) + abs(c)
...
def main() -> None:
    ...
    for vertex, distance in distances.items():
        for hacked_neighbor, distance2 in hacked_neighbors(vertex, board):
            if distances[hacked_neighbor] >= distance + distance2 + 100:
                answer += 1
